...
public class CliIntegrationTests : IDisposable
{
    ...
    private static readonly string ProjectPath = Path.GetFullPath(
        Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..",
            "src", "CoderPatros.Jsf.Cli", "CoderPatros.Jsf.Cli.csproj"));

    public CliIntegrationTests()
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"jsf-cli-test-{Guid.NewGuid():N}");
    ...
    public void GenerateKey_CreatesExpectedFiles(string algorithm, bool isSymmetric)
    {
        var (exitCode, stdout, _) = RunCli($"generate-key -a {algorithm} -o \"{_tempDir}\"");

        exitCode.Should().Be(0);

        if (isSymmetric)
        {
            var keyFile = Path.Combine(_tempDir, $"{algorithm}-symmetric.jwk");
            File.Exists(keyFile).Should().BeTrue();
            var obj = JsonNode.Parse(File.ReadAllText(keyFile))!.AsObject();
            obj["kty"]!.GetValue<string>().Should().Be("oct");
        }
        else
        {
            var privateFile = Path.Combine(_tempDir, $"{algorithm}-private.jwk");
            var publicFile = Path.Combine(_tempDir, $"{algorithm}-public.jwk");
    ...
    {
        var isSymmetric = algorithm.StartsWith("HS");
        ...
        // Sign
        string keyForSign = isSymmetric
            ? Path.Combine(_tempDir, $"{algorithm}-symmetric.jwk")
            : Path.Combine(_tempDir, $"{algorithm}-private.jwk");
        var signedFile = Path.Combine(_tempDir, "signed.json");

        var (signExit, signedOutput, _) = RunCli(
            $"sign -a {algorithm} -k \"{keyForSign}\" -i \"{inputFile}\"");
        signExit.Should().Be(0);
        File.WriteAllText(signedFile, signedOutput);

        // Verify
        string keyForVerify = isSymmetric
            ? Path.Combine(_tempDir, $"{algorithm}-symmetric.jwk")
            : Path.Combine(_tempDir, $"{algorithm}-public.jwk");
    ...
    public void SignWithEmbeddedPublicKey_VerifyWithoutExplicitKey()
    {
        ...
        var keyForSign = Path.Combine(_tempDir, $"{algorithm}-private.jwk");
        var (signExit, signedOutput, _) = RunCli(
            $"sign -a {algorithm} -k \"{keyForSign}\" --embed-public-key -i \"{inputFile}\"");
        signExit.Should().Be(0);
    ...
    public void Verify_TamperedDocument_ReturnsInvalid()
    {
        ...
        var keyForVerify = Path.Combine(_tempDir, $"{algorithm}-public.jwk");
        var (verifyExit, verifyOutput, _) = RunCli(
