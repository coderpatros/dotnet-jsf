...
internal sealed class RsaPkcs1Algorithm : ISignatureAlgorithm
{
    ...
    public RsaPkcs1Algorithm(string algorithmId, HashAlgorithmName hashAlgorithm)
    {
    ...
    public byte[] Sign(ReadOnlySpan<byte> data, SigningKey key)
    {
        ...
        return rsa.SignData(data.ToArray(), _hashAlgorithm, RSASignaturePadding.Pkcs1);
    }

    public bool Verify(ReadOnlySpan<byte> data, ReadOnlySpan<byte> signature, VerificationKey key)
    {
        var rsa = ResolveKey(key);
        return rsa.VerifyData(data.ToArray(), signature.ToArray(), _hashAlgorithm, RSASignaturePadding.Pkcs1);
    }
    ...
    private static RSA ResolveKey(VerificationKey key)
    {
        return key.KeyMaterial switch
        {
            ...
            _ => throw new ArgumentException("Invalid key type for RSA verification.")
        };
    }
}
