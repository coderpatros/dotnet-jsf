...
internal sealed class BouncyCastleRsaPssAlgorithm : ISignatureAlgorithm
{
    public string AlgorithmId { get; }
    private readonly string _hashAlgorithm;
    private const int MinimumRsaKeySizeBits = 2048;

    public BouncyCastleRsaPssAlgorithm(string algorithmId, string hashAlgorithm)
    {
    ...
    private PssSigner CreatePssSigner()
    {
        var digest = CreateDigest();
        var saltLength = digest.GetDigestSize();
        return new PssSigner(new RsaBlindedEngine(), digest, saltLength);
    }
    ...
    private static (byte[] Modulus, byte[] Exponent) ResolvePublicKey(VerificationKey key)
    {
        return key.KeyMaterial switch
        {
            ...
            _ => throw new JsfException("Invalid key type for BouncyCastle RSA-PSS verification.")
        };
    }
}
